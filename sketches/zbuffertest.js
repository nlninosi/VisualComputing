...
An infinite ladder, with some nice focal blur.

How this works:

1. I made a custom framebuffer class so that we can render a frame
   and access the depth values for each pixel. Step 1 is to draw
   an unblurred image to this framebuffer.

2. I wrote a shader that uses the depth values to apply blur to
   the image. Step 2 is to combine the unblurred image + depth info
   into a blurred final image using this shader.
